<section style="padding-top: 20px;">
  <div class="container">
    @if($carts->count() > 0)
      ...
      @endphp
      {!! Form::model($carts, ['method' => 'PUT', 'route' => ['cart.update', $carts[0]->id], 'id' => 'formId'.$carts[0]->id]) !!}
        @foreach($carts as $cart)
          ...
          <div class="row shopping-cart-table-wrap space30 {{$expressId == $cart->id ? 'selected' : ''}}" id="cartId{{$cart->id}}" data-cart="{{$cart->id}}">
              ...
              {{ Form::hidden('handling_cost[]', 0, ['id' => 'handling-cost'.$cart->id]) }}
              <div class="col-md-9 nopadding">
                  <div class="shopping-cart-header-section">
                    ...
                    <span class="pull-right">
                        @lang('theme.ship_to'):
                        <select name="ship_to[{{$cart->id}}]" class="selectBoxIt ship_to" id="shipTo{{$cart->id}}" data-cart="{{$cart->id}}">
                          {!! $country_dropdown !!}
                        </select>
                    </span>
                  </div>
                        ...
                        @foreach($cart->inventories as $item)
                          ...
                          <tr class="cart-item-tr">
                            <td>
                              ...
                              {{ Form::hidden('shipping_weight['.$cart->id.']['.$item->id.']', ($item->shipping_weight * $item->pivot->quantity), ['id' => 'itemWeight'.$item->id, 'class' => 'itemWeight'.$cart->id]) }}
                              <img src="{{ get_product_img_src($item, 'medium') }}" class="img-mini" alt="{{ $item->slug }}" title="{{ $item->slug }}" />
                              {{-- <img src="{{ get_storage_file_url(optional($item->image)->path, 'mini') }}" class="img-mini" alt="{{ $item->slug }}" title="{{ $item->slug }}" /> --}}
                            </td>
                            <td>
                              @if($item->pivot->unit_price != $unit_price)
                                <div class="notice notice-info notice-sm" style="display: block">
                                  <strong>Price for this product has been updated!</strong><br>
                                  <span>Price for this product has been {{ ($item->pivot->unit_price < $unit_price ? 'Increased' : 'Decreased') }} to {{number_format($unit_price, 2, '.', '')}} from {{number_format($item->pivot->unit_price, 2, '.', '')}} by Vendor</span>
                                </div>
                              @endif
                            ...
                            <td>
                              <div class="product-info-qty-item">
                                ...
                                <input name="quantity[{{$cart->id}}][{{$item->id}}]" id="itemQtt{{$item->id}}" class="product-info-qty product-info-qty-input" data-cart="{{$cart->id}}" data-item="{{$item->id}}" data-min="{{$item->min_order_quantity}}" data-max="{{$item->stock_quantity}}" type="text" value="{{$item->pivot->quantity}}">
                        ...
                        <tr>
                          ...
                          <td colspan="2">
                            <div class="input-group full-width">
                              ...
                              <input name="coupon[{{$cart->id}}]" value="{{ $cart->coupon ? $cart->coupon->code : Null }}" id="coupon{{$cart->id}}" class="form-control flat" type="text" placeholder="@lang('theme.placeholder.have_coupon_from_seller')">
              ...
              <div class="col-md-3 space20">
                  ...
                  <button class="btn btn-primary btn-sm flat pull-right hidden" id="checkout-btn{{$cart->id}}" type="submit"><i class="fa fa-shopping-cart"></i> {{ trans('theme.button.buy_from_this_seller') }}</button>
              </div> <!-- /.col-md-3 -->
            
          </div> <!-- /.row -->
        @endforeach
        @auth('customer')
          <button class="btn btn-primary btn-sm flat pull-right " id="checkout-btn" type="submit"><i class="fa fa-shopping-cart"></i> Proceed to Checkout</button>
        @else
          <button type="button" data-toggle="modal" data-target="#loginModal" class="btn btn-primary btn-sm flat pull-right " id="checkout-btn" ><i class="fa fa-shopping-cart"></i> Proceed to Checkout</button>
        @endauth
      {!! Form::close() !!}
