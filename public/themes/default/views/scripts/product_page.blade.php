...
;(function($, window, document) {
    ...
    var unitPrice = <?php echo $item->currnt_sale_price() ?>;
    var variants = '<?=$variants;?>';
    var itemWrapper = $("#single-product-wrapper");
    var buyNowBaseUrl = null;
    var addToCartBaseUrl = null;
    if(loggedIn == 1)
    {
        
        buyNowBaseUrl = $("#buy-now-btn").attr('href');
        buyNowBaseUrl = buyNowBaseUrl.substr(0, buyNowBaseUrl.lastIndexOf('/') + 1);

        addToCartBaseUrl = itemWrapper.find('.sc-add-to-cart').data('link');
