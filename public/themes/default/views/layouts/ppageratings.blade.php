<div class="product-info-rating text-left">
	@if($ratings)
		@for($i = 0; $i < 5; $i++)
			@if( ($ratings - $i) >= 1 )
	...
	@if(isset($count) && $count)
		@if(isset($item))
			<a href="{{ route('show.store', $item->shop->slug) . '#reviews_tab' }}" class="product-info-rating-count">({{ get_formated_decimal($ratings,true,1) }}) {{ trans_choice('theme.reviews', $count, ['count' => $count]) }}</a>
		@else
