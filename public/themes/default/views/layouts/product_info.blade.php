<div class="product-info">
  ...
  <div class="by space10">
	...
	<div class="ain space10">
    <p>{{ $variations[0]->ain != null ? 'AIN: '.$variations[0]->ain : ''}}</p>
  </div>
    ...
    <div class="col-md-4 flag-inner product-info-availability">
        <div class="flag-image">
          <span>Made In: </span>
          @if(isset($country[0]))
          <img data-toggle="tooltip" data-placement="top" title="{{$country[0]['name']}}" src="{{url('/images/flags/'.$country[0]['flag'])}}" alt="" style="height: 15px">
          @endif
        </div>
      </div>
...
<div class="select-box-wrapper">
    ...
    <div class="row product-attribute">
      <div class="col-sm-12 col-xs-12 nopadding-left space10">
        <span class="info-label" id="attr-{{str_slug($attribute->name)}}">{{ $attribute->name }}:</span>
      </div>
      <div class="col-sm-4 col-xs-12 nopadding-left">
        <select
          class="product-attribute-selector {{ $attribute->attribute_type_id == \App\Attribute::TYPE_COLOR ? 'color-options' : 'selectBoxIt' }}"
          required="required">
          @foreach($attribute->attributeValues as $option)
          <option value="{{ $option->id }}" data-color="{{ $option->color ?? $option->value }}"
            {{ in_array($option->id, $item_attrs) ? 'selected' : '' }}>{{ $option->value }}</option>
          @endforeach
        </select>
