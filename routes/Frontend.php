...
Route::group(['middleware' => ['storefront'], 'namespace' => 'Storefront'], function(){
   ...
   // Auth route for customers
	...
	Route::get('product/reviews/{slug}', 'HomeController@reviews')->name('show.product.reviews');
	Route::get('product/questions/{slug}', 'HomeController@questions')->name('show.product.questions');
	Route::post('product/ask', 'HomeController@askQuestion')->name('ask.question');
