...
use App\Cfunctions\Ain;

class EloquentInventory extends EloquentRepository implements BaseRepository, InventoryRepository
{
	protected $model;

	public function __construct(Inventory $inventory, Variations $variations)
	{
        $this->model = $inventory;
        $this->variationModel = $variations;
	}
    ...
    public function store(Request $request)
    {
        ...
        $ain = new Ain();
        $request['ain'] = $ain->generatep(12);
        $request['shop_id'] = $request->user()->merchantId(); //Set shop_id
        
        $inventory = parent::store($request);
        $variation = Variations::create($request->all());
